...
const { overlay: gameOverOverlay, playAgainButton, chooseDifficultyButton, finalScore, Highest_Score, leaderboardList } = createGameOverScreen();
...
function animate() {
    ...
    for (let i = monsters.length - 1; i >= 0; i--) {
        ...
        if (monster.checkCollision(new THREE.Vector3(camera.position.x, 0, camera.position.z))) {
            ...
            /*let highestscore = await GameFunction.GetHighestScore(playerid).then(()=>{
