...
SignUpConfirm.addEventListener("click", function (event) {
  ...
  fetch(`http://localhost/GameMain/GeneralLogin.php`, {
    method: "POST",
    headers: {
...
SignInConfirm.addEventListener("click", function (event) {
  event.preventDefault();
  fetch(
    `http://localhost/GameMain/GeneralLogin.php?action=Login&email=${LoginEmail.value}&password=${LoginPassword.value}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
